...
_PROMPT = (
    ...
    '只输出 JSON，格式：{{"categories": ["大类1", ...], "terms": ["关键词1", ...]}}，'
    "terms 不超过 20 个、每个 2-12 字、不要重复用户原词。"
)
...
def expand_majors_semantic(majors: List[str]) -> dict:
    """返回 {'categories': [...], 'terms': [...], 'source': 'ai'|'rules'}。"""
    majors = [m.strip() for m in majors if m and m.strip()][:5]
    if not majors:
        return {"categories": [], "terms": [], "source": "rules"}
    key = "ai_major_expand:" + hashlib.md5("|".join(sorted(majors)).encode()).hexdigest()
    try:
    ...
    if DEEPSEEK_API_KEY:
        try:
            ...
            terms = [str(t).strip() for t in data.get("terms", []) if 2 <= len(str(t).strip()) <= 12]
            terms = [t for t in dict.fromkeys(terms) if t not in majors][:MAX_TERMS]
            if terms:
                result = {"categories": cats, "terms": terms, "source": "ai"}
        except Exception:
            result = None
    if result is None:
        cats, terms = _rules_expand(majors)
        result = {"categories": cats, "terms": terms, "source": "rules"}
    try:
