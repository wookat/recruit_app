...
function fmt(d: Date): string {
  return tt`${d.getMonth() + 1}月${d.getDate()}日`
}
...
export function ApplyTimeline({ start, end }: { start: Date; end: Date }) {
  ...
  if (toStart > 0) {
    ...
    status = toEnd === 0 ? t("网申进行中 · 今天截止") : tt`网申进行中 · 剩 ${toEnd} 天`
  ...
  return (
    <div className="rounded-lg border border-border bg-muted/40 px-3 py-2.5">
      ...
      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-border" role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} aria-label={t("网申进度")}>
        <div
          className={cn(
      ...
      <div className="mt-1 flex justify-between text-[11px] text-muted-foreground">
        <span>{t("开始")}{' '}{fmt(start)}</span>
        <span>{t("截止")}{' '}{fmt(end)}</span>
      </div>
    </div>
  )
}
