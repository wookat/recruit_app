...
export function CompetitionRef({
  ...
  return (
    <div className="flex items-start gap-1.5 rounded-lg border border-primary/20 bg-primary/5 px-3 py-2">
      ...
      <div>
        <div className="text-xs font-medium text-muted-foreground">竞争参考</div>
        <div className="mt-0.5 text-sm">
          {year} 年{province} · {examType} 同组岗位共{' '}
          <span className="font-semibold text-primary">{data.total.toLocaleString()}</span> 个
          {data.unlimited_major > 0 && (
            <>
              ，其中不限专业约 <span className="font-semibold text-primary">{pct}%</span>
            </>
          )}
        </div>
      </div>
