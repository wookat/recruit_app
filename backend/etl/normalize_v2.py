...
_EXAM_TYPE_RULES: List[Tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"(监狱|戒毒|法官|检察官|公安|警察|警务|执法勤务|司法行政)"), "公安司法"),
    (re.compile(r"金融监管"), "金融监管类"),
    (re.compile(r"(三支一扶|乡村振兴|基层人社|西部计划|特岗)"), "基层项目"),
    (re.compile(r"^(管理岗?|管理岗位)\s*([0-9一二三四五六七八九十]+级.*|类.*)?$", re.S), "管理岗"),
    (re.compile(r"专业技术|专技"), "专业技术岗"),
    (re.compile(r"^(十一|十二|十三|1[1-3])级"), "专业技术岗"),  # 事业单位11-13级仅存在于专技岗
    (re.compile(r"^工勤|技能"), "工勤技能岗"),
