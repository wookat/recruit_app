...
def backfill_xduim() -> dict:
    ...
    try:
        ...
        for src in prov_urls:
            ...
            if not years_cities:
                print(f"[skip] {pinyin}: 无城市列表（可能非省考板块）")
                continue
            code_to_emp = {}
            for year, cities in years_cities.items():
                for city in cities:
                    ...
                    while page <= total_pages:
                        ...
                        time.sleep(0.2)
            if not code_to_emp:
                print(f"[skip] {pinyin}: 未取到 代码→单位 映射")
                continue
            filled = 0
            targets = (
            ...
            for pid, pos_example in targets:
                ...
                if emp:
                    ...
                    if filled % BATCH == 0:
                        db.commit()
            db.commit()
            stats["provinces"][pinyin] = {"map_size": len(code_to_emp), "filled": filled}
            stats["filled"] += filled
            print(f"[xduim] {pinyin}: 映射 {len(code_to_emp)} 条，回填 {filled} 条")
    finally:
        db.close()
    return stats
