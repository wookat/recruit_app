...
export function deriveCampusTags(j: CampusJob): JobTag[] {
  const tags: JobTag[] = []
  if (has(j.no_exam, '免')) {
    tags.push({ key: 'noexam', label: '免笔试' })
  }
  if ((j.referral_code || '').trim()) {
    tags.push({ key: 'referral', label: '有内推码' })
  }
