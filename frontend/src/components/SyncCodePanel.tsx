...
export function SyncCodePanel({ onClose }: { onClose: () => void }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setMsg({ ok: false, text: e instanceof Error ? e.message : t("生成失败") })
    }
  }
  ...
  const doImport = async () => {
    ...
    try {
      ...
      const extras = [
        r.profileApplied ? t("画像已同步") : null,
        r.prefsApplied > 0 ? tt`偏好 ${r.prefsApplied} 项` : null,
      ].filter(Boolean)
      setMsg({
        ok: true,
        text: tt`已合并：体制内 ${r.positions} · 校招 ${r.campus} · 编制 ${r.bianzhi}，新增 ${r.added} 条 · 更新 ${r.updated} 条${extras.length ? `（${extras.join('，')}）` : ''}`,
      })
      setPasted('')
    } catch (e) {
      setMsg({ ok: false, text: e instanceof Error ? e.message : t("导入失败") })
    }
  }
  ...
  return (
    <div className="rounded-lg border bg-muted/40 p-3">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-sm font-semibold">{t("多设备同步码")}</span>
        <span className="text-xs text-muted-foreground">{t("仅本机数据 · 无需账号")}</span>
        <Button
          variant="ghost"
          size="icon"
          aria-label={t("关闭多设备同步码")}
      ...
      <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
        {t("把收藏、投递进度、画像与偏好打包成一段文本码，在另一台设备粘贴导入即可合并（不覆盖已有数据）。")}{' '}</p>
      <Button
        variant="outline"
        size="sm"
      ...
      {tooLarge && (
        <div className="mt-2 space-y-1.5 text-xs text-amber-700 dark:text-amber-300">
          ...
          >
            ...
            {t("下载备份 JSON（另一设备用「恢复备份」导入）")}{' '}</Button>
        </div>
      )}
      {code && !tooLarge && (
        <textarea
      ...
      <div className="mt-2 space-y-1.5">
        <textarea
          ...
          rows={2}
          placeholder={t("粘贴另一设备生成的同步码（SC1: 开头）")}
